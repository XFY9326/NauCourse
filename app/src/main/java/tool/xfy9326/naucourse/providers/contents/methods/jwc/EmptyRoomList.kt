...
object EmptyRoomList : BaseNoParamContent<EmptyRoomInfo>() {
    ...
    override fun onParseData(content: String): EmptyRoomInfo {
        ...
        for ((i, element) in campusNameTag.withIndex()) {
            campusNameArr[i] = Pair(element.attr(Constants.HTML.ELEMENT_ATTR_VALUE), element.text().trim())
        }
        ...
        for ((i, element) in bjcTag.withIndex()) {
            bjcArr[i] = EmptyRoomInfo.Time(element.attr(Constants.HTML.ELEMENT_ATTR_VALUE).toInt(), element.text().trim())
        }
        ...
        for ((i, element) in ejcTag.withIndex()) {
            ejcArr[i] = EmptyRoomInfo.Time(element.attr(Constants.HTML.ELEMENT_ATTR_VALUE).toInt(), element.text().trim())
        }
