...
object StudentCardBalance : BaseNoParamContent<CardBalance>() {
    ...
    private val STUDENT_CARD_BALANCE_URL = HttpUrl.Builder().scheme(NetworkConst.HTTP).host(YktClient.YKT_HOST).addPathSegment(SELF_SEARCH_PATH)
    ...
    override fun onParseData(content: String): CardBalance {
        ...
        val li = personalNews.getElementsByTag(HTMLConst.ELEMENT_TAG_LI)
        val main = li[1].getElementsByTag(HTMLConst.ELEMENT_TAG_SPAN)[1].text().trim()
        val support = li[2].getElementsByTag(HTMLConst.ELEMENT_TAG_SPAN)[1].text().trim()
