...
object SchoolCalendarList : BaseNoParamContent<Array<CalendarItem>>() {
    ...
    private const val SELECT_CALENDAR_PAGE_PATH =
        "${HTMLConst.ELEMENT_TAG_SPAN}[${HTMLConst.ELEMENT_ATTR_CLASS}=${ELEMENT_CLASS_COL_TITLE}]"

    private val CALENDAR_LIST_PAGE_URL = HttpUrl.Builder().scheme(NetworkConst.HTTPS).host(
    ...
    private fun getCalendarListMap(bodyElement: Element): Array<CalendarItem> {
        ...
        for ((i, spanElement) in spanElements.withIndex()) {
            val aElement = spanElement.select(HTMLConst.SELECT_A_HREF_PATH_URL).first()
            calendarMap[i] = CalendarItem(aElement.text(), aElement.absUrl(HTMLConst.ELEMENT_ATTR_HREF).toHttpUrl())
        }
