...
char *AES_128_ECB_PKCS5Padding_Encrypt(const char *in, char *key) {
    ...
    int paddingInputLength = 0;
    int group = inLength / 16;
    int size = 16 * (group + 1);
    paddingInput = (uint8_t *) malloc((size_t) size);
    paddingInputLength = size;
    ...
    int count = paddingInputLength / 16;

    char *out = (char *) malloc((size_t) paddingInputLength);
    for (i = 0; i < count; ++i) {
        AES128_ECB_encrypt(paddingInput + i * 16, (const uint8_t *) key, (uint8_t *) (out + i * 16));
    }
    char *base64En = b64_encode((const unsigned char *) out, (size_t) paddingInputLength);
...
char *AES_128_ECB_PKCS5Padding_Decrypt(const char *in, char *key) {
    ...
    size_t i;
    for (i = 0; i < count; ++i) {
        AES128_ECB_decrypt(inputDesBase64 + i * 16, (const uint8_t *) key, out + i * 16);
    }
